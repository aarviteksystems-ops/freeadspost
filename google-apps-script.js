 ...
 *
 * IMPORTANT:
 * - This version NEVER stores a plain-text password (stores SHA-256 hash).
 * - Deploy as Web app: Execute as Me, Who has access: Anyone.
 * - Put the deployment URL in Vercel / .env as GOOGLE_SHEET_WEBHOOK_URL.
 * - Always call SpreadsheetApp.flush() to guarantee immediate storage in Google Sheets.
 */
...
function getSpreadsheet_() {
  ...
  throw new Error('No active Google Spreadsheet found. Ensure this script is created inside your Google Sheet (Extensions > Apps Script) or configure SPREADSHEET_ID in Script Properties.');
}
...
function sheet_(name, headers) {
  ...
  if (!s) {
    ...
    if (defaultSheet && defaultSheet.getLastRow() === 0 && name === USERS_SHEET) {
      s = defaultSheet;
      s.setName(name);
    } else {
...
function userFromRow_(r) {
  return {
    ...
    passwordHash: String(r[5] || ''),
    createdAt: String(r[6] || ''),
    lastActiveAt: String(r[7] || '') || null,
    availabilityStatus: String(r[8] || 'away'),
...
function adFromRow_(r) {
  ...
  return {
    ...
    title: String(r[2] || ''),
    category: String(r[3] || ''),
    condition: String(r[4] || 'not-applicable'),
    description: String(r[5] || ''),
    price: String(r[6] || '0'),
    priceType: String(r[7] || 'fixed'),
    city: String(r[8] || ''),
    state: String(r[9] || ''),
    image: String(r[10] || ''),
    images,
    postedAt: String(r[12] || ''),
    status: String(r[13] || 'published')
  };
}
...
function findAd_(id) {
  return rows_(ads_()).map(adFromRow_).find(a => a.id === String(id)) || null;
}
...
function updateUser_(user) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]) === String(user.id)) {
      s.getRange(i + 1, 1, 1, 10).setValues([[
        user.id,
        user.name,
        user.email,
...
function sellerSafeAd_(ad, seller) {
  ...
  return {
    ...ad,
    sellerAvailable: isAvailable,
    seller: seller ? { id: seller.id, name: seller.name, companyName: seller.companyName || '' } : undefined
  };
}
...
function ensureSeed_() {
  ...
  samples.forEach((x, i) => {
    ...
    if (!findUser_(sellerId)) {
      us.appendRow([sellerId, x[1].split('–')[0].trim(), email, '', '', sha256_('demo12345'), new Date(now - i * 86400000).toISOString(), now_(), 'available', 'active']);
    }
...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'health') {
      ...
      try {
        ...
        sheetInfo = {
          ...
          usersCount: rows_(users_()).length,
          adsCount: rows_(ads_()).length
        };
      } catch (_) {}
      return json_({ status: 'success', storage: 'google-sheets', ...sheetInfo });
    }
    ...
    if (action === 'login') {
      ...
      if (!user || user.passwordHash !== sha256_(p.password)) {
        const err = new Error('Invalid email or password');
        err.code = 401;
        throw err;
      }
      user.lastActiveAt = now_();
    ...
    if (action === 'getAd') {
      ...
      if (!ad || !seller || seller.status !== 'active' || ['sold','paused'].includes(ad.status)) {
        const err = new Error('Ad is currently unavailable');
        err.code = 404;
        throw err;
      }
    ...
    if (action === 'createAd') {
      ...
      const ad = {
        id: 'ad-' + Utilities.getUuid(),
        sellerId: seller.id,
        title,
        category: String(p.category),
        condition: String(p.condition || 'not-applicable'),
        description,
        price: String(p.price || '0'),
        priceType: String(p.priceType || 'fixed'),
        city,
    ...
    if (action === 'deleteAd') {
      ...
      if (ad.sellerId !== String(p.userId)) {
        const err = new Error('You can only delete your own ads');
        err.code = 403;
        throw err;
      }
      const s = ads_(), values = s.getDataRange().getValues();
      for (let i = values.length - 1; i >= 1; i--) {
        if (String(values[i][0]) === String(p.id)) s.deleteRow(i + 1);
      }
      SpreadsheetApp.flush();
...
function doGet() {
  try {
    const ss = getSpreadsheet_();
    const uCount = rows_(users_()).length;
    const aCount = rows_(ads_()).length;
    return json_({
      status: 'ok',
      message: 'FreeAdsPost storage API is active',
      spreadsheetName: ss.getName(),
      spreadsheetId: ss.getId(),
      spreadsheetUrl: ss.getUrl(),
      registeredUsers: uCount,
      totalAds: aCount
    });
  } catch (err) {
    return json_({ status: 'error', error: String(err.message || err) });
  }
}
